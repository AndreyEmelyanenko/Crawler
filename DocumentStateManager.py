...
class DocumentStateManager(object):
    ...
    def __init__(self, url):
        ...
        if self.document != None:
            print("OK")
            self.data = self.document['_source']['data']
            self.id = self.document['_source']['key']
        else:
            print("NOT OK")
            self.data = None
            self.id = None
    ...
    def ChangeState(self, NewIndex):
        ...
        try:
            print("ChangeState")
            res = self.es.index(index=NewIndex, doc_type="all", id=self.id, body=self.document['_source'], refresh=True)
            print("delete")
            self.es.delete(index=self.bufferindex, doc_type="all", id=self.id)
            print(res)
            return res
...
if __name__ == "__main__":
    ...
    if DocumentStateManager.data:
        print(DocumentStateManager.data)
    if DocumentStateManager.id:
